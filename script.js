document.addEventListener("DOMContentLoaded", () => {
  ...
  const convite = {
    ...
    mensagemInterna: "Hakuna Matata! Todo o reino te espera para comemorar esse dia especial! 🦁",
    ...
    // Trilha sonora — começa somente depois do toque no envelope.
    musicaVolume: 0.22,
    musicaFadeMs: 1900
  };
  ...
  function configurarVideo() {
    ...
    const fallbackTimer = setTimeout(() => {
      if (el.heroVideo.readyState < 2) el.heroFallback?.classList.add("is-visible");
    }, 2500);
    ...
    el.heroVideo.addEventListener("error", () => el.heroFallback?.classList.add("is-visible"));
  ...
  function atualizarControleMusica(estado) {
    if (!el.musicControl) return;

    const tocando = estado === "playing";
    el.musicControl.classList.toggle("is-playing", tocando);
    el.musicControl.classList.toggle("is-ended", estado === "ended");
    el.musicControl.setAttribute("aria-pressed", String(tocando));

    if (estado === "playing") {
      el.musicControl.setAttribute("aria-label", "Pausar trilha da savana");
      setText(el.musicStatus, "tocando baixinho");
    } else if (estado === "ended") {
      el.musicControl.setAttribute("aria-label", "Tocar trilha da savana novamente");
      setText(el.musicStatus, "tocar novamente");
    } else if (estado === "blocked") {
      el.musicControl.setAttribute("aria-label", "Tocar trilha da savana");
      setText(el.musicStatus, "toque para tocar");
    } else {
      el.musicControl.setAttribute("aria-label", "Continuar trilha da savana");
      setText(el.musicStatus, "pausada");
    }
  }

  function fadeMusica(ate, duracao = 700, aoTerminar) {
    if (!el.bgMusic) return;
    if (musicFadeFrame) cancelAnimationFrame(musicFadeFrame);

    const inicio = performance.now();
    const de = el.bgMusic.volume;
    const destino = Math.min(1, Math.max(0, ate));

    const passo = (agora) => {
      const progresso = Math.min(1, (agora - inicio) / duracao);
      // ease-out suave: sobe rápido no começo e termina sem degrau.
      const suave = 1 - Math.pow(1 - progresso, 3);
      el.bgMusic.volume = de + (destino - de) * suave;

      if (progresso < 1) {
        musicFadeFrame = requestAnimationFrame(passo);
      } else {
        musicFadeFrame = null;
        aoTerminar?.();
      }
  ...
  function tocarMusica({ reiniciar = false } = {}) {
    if (!el.bgMusic) return;

    if (reiniciar || el.bgMusic.ended) el.bgMusic.currentTime = 0;
    if (musicFadeFrame) cancelAnimationFrame(musicFadeFrame);
    el.bgMusic.volume = Math.min(el.bgMusic.volume || 0.01, 0.04);

    const tentativa = el.bgMusic.play();
    if (tentativa && typeof tentativa.then === "function") {
      tentativa.then(() => {
        atualizarControleMusica("playing");
        fadeMusica(convite.musicaVolume, convite.musicaFadeMs);
      }).catch(() => {
        atualizarControleMusica("blocked");
      });
    } else {
      atualizarControleMusica("playing");
      fadeMusica(convite.musicaVolume, convite.musicaFadeMs);
  ...
  function iniciarMusicaAoAbrir() {
    if (!el.bgMusic || !el.musicControl) return;
  ...
  function configurarMusica() {
    ...
    el.musicControl.addEventListener("click", () => {
      if (el.bgMusic.paused || el.bgMusic.ended) {
        tocarMusica({ reiniciar: el.bgMusic.ended });
        return;
      }

      atualizarControleMusica("paused");
      fadeMusica(0, 320, () => el.bgMusic?.pause());
    });

    el.bgMusic.addEventListener("ended", () => {
      atualizarControleMusica("ended");
    });
  }

  function soltarCelebracao() {
    if (!el.celebration || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    el.celebration.replaceChildren();
    const total = 22;
    for (let i = 0; i < total; i += 1) {
      const p = document.createElement("span");
      const r = (360 / total) * i + Math.random() * 10;
      const d = 95 + Math.random() * 125;
      p.style.setProperty("--r", `${r}deg`);
      p.style.setProperty("--d", `${d}px`);
      p.style.animationDelay = `${Math.random() * 0.08}s`;
      el.celebration.appendChild(p);
    }
    setTimeout(() => el.celebration?.replaceChildren(), 1200);
  }

  function configurarEnvelope() {
    let aberto = false;
    if (!el.btnAbrir || !el.envelope || !el.stageEnvelope || !el.stageContent) return;
    ...
    el.btnAbrir.addEventListener("click", () => {
      ...
      // O clique que abre o convite também libera a trilha sonora no celular.
      iniciarMusicaAoAbrir();
      el.envelope.classList.add("is-flap-open");
      navigator.vibrate?.(25);

      setTimeout(() => el.envelope.classList.add("is-letter-out"), 260);
      setTimeout(soltarCelebracao, 590);

      setTimeout(() => {
        el.stageEnvelope.classList.add("hidden");
        el.stageContent.classList.remove("hidden");
        el.stageContent.classList.add("is-visible");
        el.stageContent.scrollIntoView({ behavior: "smooth", block: "nearest" });
      }, 1120);
    });
  }
  ...
  function configurarPresentes() {
    ...
    const abrir = () => {
      ...
      el.modal.querySelector(".modal__close")?.focus();
    };
    const fechar = () => {
    ...
    el.btnPresentes?.addEventListener("click", abrir);
    el.modal.addEventListener("click", (event) => {
      const target = event.target;
      if (target instanceof HTMLElement && target.dataset.close === "true") fechar();
    });
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && !el.modal.classList.contains("hidden")) fechar();
    });
  }
  ...
  function configurarAlbum() {
    ...
    if (!convite.albumLink) {
      el.btnAlbum.querySelector("small")?.replaceChildren(document.createTextNode("em breve • memórias do grande dia"));
      el.btnAlbum.addEventListener("click", () => alert("O álbum estará disponível depois da festa 📸💛"));
      return;
    }
    el.btnAlbum.addEventListener("click", () => openExternal(convite.albumLink));
  }
  ...
  function preencherContagem(diff) {
    ...
    setText(el.countDays, String(dias).padStart(2, "0"));
    setText(el.countHours, String(horas).padStart(2, "0"));
    setText(el.countMinutes, String(minutos).padStart(2, "0"));
    setText(el.countSeconds, String(segundos).padStart(2, "0"));
  }
